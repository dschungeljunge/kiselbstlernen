import Link from "next/link";
import { WORKSHOP1_STEPS } from "@/lib/workshop1";

export default function Workshop1Page() {
  const totalMinutes = WORKSHOP1_STEPS.reduce((sum, step) => sum + step.durationMinutes, 0);

  return (
    <>
      <div className="relative overflow-hidden bg-gradient-to-br from-yellow-400 via-yellow-300 to-yellow-400">
        <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-30" />
        <div className="relative px-6 py-14">
          <div className="mx-auto max-w-4xl">
            <h1 className="text-4xl font-bold tracking-tight text-zinc-950 md:text-5xl">
              BBZO Workshop 1
            </h1>
            <p className="mt-3 max-w-3xl text-base leading-7 text-zinc-800 md:text-lg">
              Du gestaltest einen KI-Tutor f&uuml;r einen spezifischen Beruf. Arbeite die
              Schritte nacheinander durch und dokumentiere deine Ergebnisse direkt im Team.
            </p>
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl bg-white/75 p-4 shadow-sm ring-1 ring-white/50">
                <p className="text-xs font-semibold uppercase tracking-wide text-zinc-600">
                  Umfang
                </p>
                <p className="mt-1 text-sm font-semibold text-zinc-950">
                  {WORKSHOP1_STEPS.length} Schritte, ca. {totalMinutes} Minuten
                </p>
              </div>
              <div className="rounded-2xl bg-white/75 p-4 shadow-sm ring-1 ring-white/50">
                <p className="text-xs font-semibold uppercase tracking-wide text-zinc-600">
                  Im Fokus
                </p>
                <p className="mt-1 text-sm font-semibold text-zinc-950">
                  Lernschwierigkeiten, Berufslogik, p&auml;dagogische Wirkung
                </p>
              </div>
              <div className="rounded-2xl bg-white/75 p-4 shadow-sm ring-1 ring-white/50">
                <p className="text-xs font-semibold uppercase tracking-wide text-zinc-600">
                  Ergebnis
                </p>
                <p className="mt-1 text-sm font-semibold text-zinc-950">
                  Ein getesteter Berufs-Tutor-Prompt
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="min-h-screen bg-zinc-50 px-6">
        <main className="mx-auto w-full max-w-4xl pb-20 pt-12">
          <section className="rounded-2xl border border-yellow-300 bg-gradient-to-br from-yellow-50 to-white p-6 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-yellow-700">
              Arbeitsauftrag
            </p>
            <h2 className="mt-2 text-xl font-bold text-zinc-950">Gemeinsam einen Tutor entwerfen</h2>
            <p className="mt-3 text-base leading-7 text-zinc-700">
              Am Ende habt ihr einen getesteten Berufs-Tutor-Prompt, den ihr direkt in der
              Weiterbildung einsetzen und weiterentwickeln k&ouml;nnt.
            </p>
          </section>

          <section className="mt-6 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">
                  Ablauf
                </p>
                <h3 className="mt-1 text-lg font-semibold text-zinc-950">
                  Schritt f&uuml;r Schritt durch den Workshop
                </h3>
              </div>
              <p className="text-sm text-zinc-500">Empfohlene Zeit pro Schritt: 10 Minuten</p>
            </div>
            <ol className="mt-5 grid gap-3 sm:grid-cols-2">
              {WORKSHOP1_STEPS.map((step) => (
                <li
                  key={step.step}
                  className="rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-700"
                >
                  <p className="font-semibold text-zinc-950">
                    {step.step}. {step.title}
                  </p>
                  <p className="mt-1 text-zinc-500">{step.durationMinutes} Minuten</p>
                </li>
              ))}
            </ol>
          </section>

          <div className="mt-8 flex justify-end">
            <Link
              href="/ws1/1"
              className="inline-flex h-11 items-center justify-center rounded-xl bg-zinc-950 px-8 text-sm font-semibold text-white hover:bg-zinc-800 focus:outline-none focus:ring-4 focus:ring-zinc-200"
            >
              Workshop starten →
            </Link>
          </div>

          <div className="mt-8 rounded-lg bg-zinc-100 p-5 text-center">
            <p className="text-sm text-zinc-600">
              <Link href="/" className="font-medium text-zinc-800 hover:underline">
                ← Zur Startseite
              </Link>
            </p>
          </div>
        </main>
      </div>
    </>
  );
}

